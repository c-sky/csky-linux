...
static int __init nc_timer_init(struct device_node *np)
{
	...
	pr_info("Nationalchip Timer Init, reg: %p, irq: %d.\n",
		reg_base, irq);
	...
	/* register */
	clockevents_config_and_register(&nc_ced, TIMER_FREQ, 1, ULONG_MAX);

	nc_csd_enable();
	clocksource_mmio_init(reg_base + COUNTER_2_VALUE, "nationalchip", TIMER_FREQ, 200, 32,
